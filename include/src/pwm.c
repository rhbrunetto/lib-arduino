...
#include "pins.h"
    ...
    void pwmwave(uint8_t pin, uint8_t dutycicle) {
        set_output(Pins[pin].P);
        if ((Pins[pin]._TCCA) == NULL) {
            uint8_t pwmpin = dutycicle > 128? digital_write(Pins[pin].P, HIGH) : digital_write(Pins[pin].P, LOW);
        }
        ...
        *(Pins[pin]._TCCB) = 2;
        if (Pins[pin]._OCRB != NULL) {
            *(Pins[pin]._OCRB) = dutycicle;
        }
        else {
            *(Pins[pin]._OCRA) = dutycicle;
        }
    }
